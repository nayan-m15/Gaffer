-- Final post-merge reconciliation for the divergent development and
-- competition migration histories. The custom migration runner decides
-- pending work by the greatest journal timestamp, so long-lived databases
-- may have skipped development's projection/offline migrations even though
-- their schema is required by the merged application.
--
-- This migration intentionally replays only the idempotent repair/hardening
-- definitions. It does not drop data or rewrite migration history.
