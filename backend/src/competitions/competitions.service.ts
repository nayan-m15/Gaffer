...
export class CompetitionsService {
  ...
  /** Search by name for any signed-in coach. Search grants no membership. */
  async search(
    userId: string,
    term: string,
  ): Promise<CompetitionSummaryView[]> {
    await this.requireCoachTeam(userId);
  ...
  /**
   * Detail view for a competition reachable by any signed-in coach (search
   * must be able to open it), including its participants. Linked participants
   * expose only their id, display name and team id — never a private user
   * email.
   */
  async findOne(
    userId: string,
  ...
  /** Coach-only gate — only a team's coach may create or manage competitions. */
