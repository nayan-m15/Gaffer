...
} from "@/services/competition-invites";
...
export default function JoinCompetitionPage() {
  ...
  const doAccept = async () => {
    ...
    try {
      ...
      if (!session?.team) {
        setFormError("Invitation accepted. Please retry to refresh your team membership.");
        return;
      }
    ...
    try {
      ...
      if (emailVerificationRequired) {
        ...
        navigate("/verify-email", {
          replace: true,
          state: { email, inviteToken: token, inviteKind: "competition" },
        });
        return;
      }
      // Show the signed-in confirmation before accepting.
    } catch (err) {
  ...
  /* ── Render: loading ──────────────────────────────────────────────── */
  if (authStatus === "offline" || authStatus === "unavailable") {
    return <Shell><p role="alert" className="mb-4 text-sm text-muted-foreground">Connect to the server to continue with your invitation.</p><Button onClick={() => void refreshSession()}>Try again</Button></Shell>;
  }

  if (previewUnavailable) {
    return <Shell><p role="alert" className="mb-4 text-sm text-muted-foreground">Could not load your invitation. Please try again.</p><Button onClick={() => setRetryPreview((value) => value + 1)}>Try again</Button></Shell>;
  }

  if (previewLoading || authStatus === "loading") {
    return (
      <Shell>
      ...
      <Shell>
        <div className="flex flex-col items-center gap-6 py-4 text-center">
          <div className="flex flex-col gap-1">
            ...
            <h1 className="text-xl font-bold text-foreground">{preview.competitionName}</h1>
    ...
    <Shell>
      <div className="flex flex-col gap-5">
        ...
        <div className="text-center">
          ...
          <h1 className="mt-1 text-xl font-bold text-foreground">{preview.competitionName}</h1>
            <p className="text-sm text-muted-foreground">Team: {teamName}</p>
        ...
        {emailMismatch && (
          ...
          >
            ...
            create an account — with the email address the competition admin invited to
            join {teamName}.
          </div>
        )}
