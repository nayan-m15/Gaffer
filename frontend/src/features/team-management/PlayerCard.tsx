...
function Nameset({ text, pathId }: { text: string; pathId: string }) {
  const size = text.length <= 7 ? 36 : text.length <= 9 ? 33 : text.length <= 12 ? 29 : 25;

  return (
    <svg className="player-card__nameset" viewBox="0 0 240 60" role="img" aria-label={text}>
      <defs>
...
function Stat({
  ...
  return (
    <span className={cn("player-card__stat", value === 0 && "player-card__stat--zero")}>
      <span className="player-card__stat-value">
        {card && <i className={`player-card__booking player-card__booking--${card}`} aria-hidden="true" />}
        {value}
      </span>
  ...
  if (variant === "pitch") {
    return (
      <div
        ...
        role="button"
        aria-label={`${name} — ${displayPosition}, number ${displayNumber} · ${statusLabel}`}
        tabIndex={0}
      >
        <span className="player-card__pitch-topline">
          <span className="player-card__position">{displayPosition}</span>
          <span className="player-card__status-dot" title={statusLabel} aria-hidden="true" />
        </span>
