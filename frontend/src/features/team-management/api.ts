 ...
 * lineups; `useLineup` fetches one by id. `useCreateLineup` / `useUpdateLineup`
 * / `useDeleteLineup` persist changes so a coach can save multiple named
 * lineups and switch between them.
 */
...
export function useLineup(lineupId: string | undefined) {
  return useQuery<BackendLineup>({
    queryKey: [...lineupsQueryKey, lineupId],
